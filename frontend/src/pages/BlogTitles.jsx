...
const BlogTitles = () => {
  ...
  const blogCategories = ['General', 'Technology', 'Health', 'Lifestyle', 'Education', 'Business', 'Travel', 'Food'];
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {
            blogCategories.map((item) => (
              <span onClick={() => setSelectedLength(item)} key={item} 
                className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength === item ? 'bg-purple-50 text-purple-700' : 'text-gray-500 border-gray-300'}`}>
                {item}  
              </span>
            ))
          }
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96  '>
          ...
          <div className='flex-1 flex justify-center items-center'>
            <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
              ...
              <p>Enter a topic and hit generate to generate a Title. </p>
            </div>
          </div>
      </div>
    </div>
